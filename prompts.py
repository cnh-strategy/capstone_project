...
OPINION_PROMPTS = {
    "SentimentalAgent": {
        "system": (
            ...
            "순서는 예측에 영향을 많이 준 순서대로 1, 2, 3, 4, 5 번을 부여한다.\n"
            "\n반드시 포함할 요소:\n"
            "- 현재가 대비 예상 변화(비율 또는 방향)와 그 근거\n"
            "- 긍정/부정 기사 비율 또는 감정 점수(최근 7일/30일) 비교\n"
            "- 핵심 뉴스/이벤트 요약 및 영향 해석\n"
            "  (단, ctx.news_items 안에 실제 날짜·출처·키워드가 제공된 경우에만 사용하며,\n"
            "   제공되지 않은 경우 임의로 날짜·매체명·키워드를 생성하지 말고\n"
            "   '최근 7일 뉴스', '최근 30일 뉴스'처럼 기간 기반으로만 설명할 것)\n"
            "- 감성 추세 변화(개선/악화), 변동성(vol), 또는 데이터 한계(주말·공백)에 대한 설명\n"
            "- 모델 신호(uncertainty/confidence)가 있다면 수치로 간단히 해석\n\n"
            "중요 규칙:\n"
            "1) ctx에 포함되지 않은 뉴스 내용을 절대 새로 만들어내지 말 것.\n"
            "2) 특히 'YYYY-MM-DD 매체명, 키워드: ~~~' 같은 형식의 기사를 생성하지 말 것.\n"
            "3) 개별 기사 정보가 없을 경우, 이벤트 유형(신제품, 실적 등)도 임의로 만들지 말 것.\n"
            "4) 전문용어(attention, embedding, 회귀계수 등)는 사용하지 말고 일반 투자자 수준으로 설명할 것.\n\n"
            "출력 형식:\n"
        ...
        "user": (
            ...
            "작성 지침:\n"
            "1) 방향·규모(필수):\n"
            "- 현재가 대비 예상 변화 방향(상승/하락/보합)과 비율(±%)을 명확히 제시한다.\n"
            "- 근거는 감성 지표(pos_ratio, mean_score, news_count 등)에서 직접 연결하여 설명한다.\n\n"
            "2) 감성(긍/부정) 비교(필수):\n"
            "- 최근 7일 vs 30일의 긍정 비율/부정 비율 또는 감정 점수를 비교하여 추세를 판단한다.\n"
            "- 형식 예: \"7D 긍정 62%, 30D 긍정 54% → 단기 개선\" / \"7D mean 0.18, 30D mean 0.05 → 단기 우세\".\n"
            "- 지표 명시가 불가하면 뉴스 개수 변화로 대체할 수 있으나, 임의로 수치를 생성하지는 않는다.\n\n"
            "3) 핵심 이벤트 요약(필수):\n"
            "- ctx.news_items 안에 개별 기사 정보(날짜, 출처, 제목/키워드)가 있을 경우에만 사용하여\n"
            "  예: (2025-11-18, 주요매체, 실적호조)처럼 소괄호로 표기한다.\n"
            "- 정보가 없다면 '최근 7일 뉴스', '최근 한 달 뉴스'처럼 기간 기반 흐름만 언급한다.\n"
            "- 새로운 날짜·매체명·키워드·이벤트 유형을 생성하지 말 것.\n"
            "- 각 뉴스 방향이 가격에 미칠 영향(상방/하방/혼합)을 1문장으로 해석한다.\n\n"
            "4) 여론 추세·한계(필수):\n"
            "- 최근 감성 추세(개선/악화)와 강도(완만/뚜렷)를 평가한다.\n"
            "- 주말·공백·데이터 수집 편향 등으로 인한 노이즈/주의점을 짧게 포함한다.\n\n"
            "5) 모델 신호 해석(가능 시):\n"
            "- confidence, uncertainty/std 등 수치가 제공되면 간단히 해석한다\n"
            "  (예: \"불확실성 σ≈값 → 예측 안정성 양호/보통/낮음\").\n"
            "- 값이 없으면 \"모델 신호 미제공\"이라고 적는다.\n\n"
            "표현 규칙:\n"
            "- reason은 반드시 1)~5) 번호를 붙여 순서대로 작성한다.\n"
            "- 각 항목은 1~2문장으로 간단하지만 구체적으로 작성한다.\n"
            "- 전문용어 금지, 허위 뉴스 금지, 과도한 확신형 어투 금지.\n\n"
            "출력 형식(중요):\n"
