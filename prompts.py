...
OPINION_PROMPTS = {
    "SentimentalAgent": {
        "system": (
            ...
            "다만 순서는 예측에 영향을 많이 준 순서대로 1, 2, 3, 4, 5 번을 부여"
            "반드시 포함할 요소:\n"
            ...
            "{\"our_prediction\": number, \"reason\": string} 만 허용한다."
        ),
        "user": (
            """ctx(JSON):
        {context}
        ...
        작성 지침:
        1) 방향·규모(필수):
        - 현재가 대비 예상 변화 방향(상승/하락/보합)과 대략적 비율(±% 또는 ↑/↓)을 1문장으로 제시
        - 근거는 감성 지표 및 뉴스 흐름에서 직접 인용(예: pos_ratio, mean_score, news_count)

        2) 감성(긍/부정) 비교(필수):
        - 최근 7일 vs 30일의 긍정/부정 비율 또는 감정 점수를 비교하여 추세 판단.
        - 형식 예: "7D 긍정 62%, 30D 긍정 54% → 단기 개선" / "7D mean 0.18, 30D mean 0.05 → 단기 우세"
        - 지표 명시가 불가하면 뉴스 개수/키워드로 대체해 추세 해석(증가/감소, 강도)

        3) 핵심 이벤트(필수, 1~3개):
        - 날짜·출처·키워드를 소괄호로 간단히 표기: "납품 확대(2025-10-28, Bloomberg, 'supply deal')"
        - 각 이벤트가 가격에 미칠 가능 영향(상방/하방/혼합)을 1문장으로 해석

        4) 여론 추세·한계(필수):
        - 여론의 변화(개선/악화)와 강도(완만/뚜렷)를 판별
        - 데이터 노이즈·공백(주말, 공휴일, 단일 대형 기사 쏠림 등)으로 인한 한계/주의점을 1문장 포함

        5) 모델 신호 해석(가능 시):
        - 제공된 수치(confidence, uncertainty.std, ci95 등)가 있으면 간단 해석: "불확실성 σ≈{값}, 예측 신뢰도 낮음/보통/높음"
        - 수치가 없으면 "모델 신호 미제공"으로 간단 표기

        표현 규칙:
        - reason은 반드시 1)~5) 번호를 붙여 순서대로 작성(각 항목 1~2문장 권장)
        - 전문용어(예: attention, embedding, 회귀계수, SHAP 등) 금지. 과도한 확신형 어투 금지
        - 날짜는 YYYY-MM-DD, 비율은 소수 1자리(예: 3.2%), 수치 인용 시 ctx 값만 사용(임의 계산 금지)
