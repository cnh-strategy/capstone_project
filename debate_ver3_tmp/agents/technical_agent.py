...
import pandas as pd
import os
from debate_ver3_tmp.agents.base_agent import BaseAgent, StockData, Target
...
class MacroAgent(BaseAgent, nn.Module):
    ...
    def __init__(self, 
    agent_id="MacroAgent",
    input_dim=agents_info["MacroAgent"]["input_dim"],
    hidden_dim=agents_info["MacroAgent"]["hidden_dim"],
    dropout=agents_info["MacroAgent"]["dropout"],
    data_dir=dir_info["data_dir"],
    window_size=agents_info["TechnicalAgent"]["window_size"],
    epochs=agents_info["TechnicalAgent"]["epochs"],
    learning_rate=agents_info["MacroAgent"]["learning_rate"],
