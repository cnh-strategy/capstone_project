...
class MultiTickerDatasetBuilder:
    """Multi-ticker dataset builder for various stocks"""
    ...
    def __init__(self, data_dir='data'):
        ...
        self.popular_tickers = {
            ...
            'Korean': ['005930.KS', '000660.KS', '035420.KS', '207940.KS', '006400.KS']
        }
    
    def get_available_tickers(self):
        """Get list of available tickers by category"""
    ...
    def build_sentimental_dataset(self, ticker, period="4y"):
        """Build sentimental analysis dataset"""
        ...
        try:
            ...
            price_volatility = df_price["returns"].rolling(20).std().fillna(0)
            
            # Normalize volatility
            if price_volatility.std() > 0:
                price_volatility_norm = (price_volatility - price_volatility.mean()) / price_volatility.std()
            else:
                price_volatility_norm = pd.Series(0, index=price_volatility.index)
            
            # Calculate sentiment scores
            sentiment_mean = (np.tanh(price_momentum * 10) * 0.5 + 
                             np.tanh(volume_change * 5) * 0.3 + 
                             -price_volatility_norm * 0.2)
            
            # Calculate sentiment volatility
            sentiment_vol = sentiment_mean.rolling(20).std().fillna(0)
            
            # Add to dataframe - ensure we're working with Series
    ...
    def build_multiple_tickers(self, tickers):
        ...
        for i, ticker in enumerate(tickers, 1):
            ...
            if self.build_datasets_for_ticker(ticker):
...
def main():
    """Main function for multi-ticker dataset building"""
    builder = MultiTickerDatasetBuilder()
    
    # Show available tickers
    print("📊 Available Ticker Categories:")
    for category, tickers in builder.get_available_tickers().items():
        print(f"   {category}: {', '.join(tickers)}")
    
    # Example: Build datasets for a few popular tickers
    example_tickers = ['AAPL', 'MSFT', 'GOOGL']
    
    print(f"\n🎯 Building datasets for example tickers: {', '.join(example_tickers)}")
    
    successful, failed = builder.build_multiple_tickers(example_tickers)
