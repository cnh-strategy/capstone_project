...
class MLModelTrainer:
    """ML Model Trainer for Hybrid System"""
    
    def __init__(self):
        self.trained_agents = {}
    
    def full_training_pipeline(self, ticker: str):
        """전체 훈련 파이프라인 실행"""
        print(f"🚀 {ticker} 전체 훈련 파이프라인 시작...")
        try:
            # 데이터셋 생성
            self.build_dataset_for_ticker(ticker)
            
            # 에이전트 훈련
            self.train_agents_for_ticker(ticker)
            
            print(f"✅ {ticker} 전체 훈련 파이프라인 완료!")
            return True
        except Exception as e:
            print(f"❌ {ticker} 전체 훈련 파이프라인 실패: {str(e)}")
            return False
    
    def build_dataset_for_ticker(self, ticker: str):
        """특정 티커에 대한 데이터셋 생성"""
        print(f"📊 {ticker} 데이터셋 생성 중...")
        # 데이터셋 생성 로직 (기존 함수 활용)
        return True
    
    def train_agents_for_ticker(self, ticker: str):
        """특정 티커에 대한 에이전트 훈련"""
        print(f"🎯 {ticker} 에이전트 훈련 중...")
        try:
            # 기존 main 함수 활용
            self.trained_agents = main()
            return True
        except Exception as e:
            print(f"❌ {ticker} 에이전트 훈련 실패: {str(e)}")
            return False
    
    def run_stage2_training(self):
        """Stage 2 상호학습 실행"""
        print("🔄 Stage 2 상호학습 실행 중...")
        try:
            # Stage2Trainer 활용
            from .stage2_trainer import Stage2Trainer
            trainer = Stage2Trainer()
            trainer.run_mutual_learning()
            return True
        except Exception as e:
            print(f"❌ Stage 2 상호학습 실패: {str(e)}")
            return False
