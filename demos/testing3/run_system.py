...
def run_streamlit(port=8501):
    """Streamlit 대시보드 실행"""
    print(f"🚀 Streamlit 대시보드 실행 중... (포트: {port})")
...
def run_cli(ticker, step, force_retrain, rounds):
    """CLI 모드 실행"""
    print(f"🎯 CLI 모드 실행: {ticker}")
...
def main():
    ...
    parser.add_argument('--step', type=str, 
                       choices=['search', 'train', 'predict', 'debate', 'all'],
    ...
    if args.mode == 'check':
        ...
        if check_dependencies():
            run_cli(args.ticker, args.step, args.force_retrain, args.rounds)
