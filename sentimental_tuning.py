# sentimental_tuning.py
# SentimentalAgent 전용 파라미터 튜닝 스크립트
...
def run_hyperparameter_tuning(
    ...
    log_file_path = os.path.join(base_output_dir, f"sentimental_tuning_log_{ticker}_{timestamp}.txt")
    ...
    try:
        print(f"🚀 Starting Random Search Tuning for {ticker} (SentimentalAgent)")
        ...
        # Case B: SentimentalAgent (Window, Model Size, LR)
        ...
        full_param_grid = {**common_grid, **sent_grid}
        ...
        # 결과 저장 경로 미리 정의
        save_path = os.path.join(base_output_dir, f"sentimental_tuning_summary_{ticker}_{timestamp}.csv")
...
if __name__ == "__main__":
    ...
    for ticker in target_tickers:
        ...
        try:
            # 티커별로 결과 폴더 분리
            ticker_output_dir = os.path.join("backtest", "sentimental_tuning_results", ticker)
    ...
    if all_best_configs:
        ...
        total_summary_path = os.path.join("backtest", "sentimental_tuning_results", f"total_best_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
