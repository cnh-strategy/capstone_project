...
class MacroSentiAgent(BaseAgent, nn.Module):
    def __init__(self,
                 agent_id="MacroSentiAgent",
                 input_dim=agents_info["MacroSentiAgent"]["input_dim"],
                 hidden_dim=agents_info["MacroSentiAgent"]["hidden_dim"],
                 num_layers=agents_info["MacroSentiAgent"]["num_layers"],
                 dropout=agents_info["MacroSentiAgent"]["dropout"],
                 data_dir=dir_info["data_dir"],
                 window_size=agents_info["MacroSentiAgent"]["window_size"],
                 epochs=agents_info["MacroSentiAgent"]["epochs"],
                 learning_rate=agents_info["MacroSentiAgent"]["learning_rate"],
                 batch_size=agents_info["MacroSentiAgent"]["batch_size"],
                 **kwargs
                 ):
    ...
    def _build_model(self):
        """MacroSentiAgent 기본 LSTM 모델 자동 생성 (학습 수렴 개선 버전)"""
        import torch.nn as nn
        ...
        print(f"🧠 MacroSentiAgent LSTM 생성 완료 "
    ...
    def _build_messages_opinion(self, stock_data, target):
        """MacroSentiAgent용 LLM 프롬프트 메시지 구성 (시계열 포함 버전)"""
