...
from torch.utils.data import DataLoader, TensorDataset # DataLoader, TensorDataset import 추가
...
# common_params 임시 설정 (코드에 없어서 BaseAgent 참조)
try:
    from config.agents import common_params # config.agents에 common_params가 있다고 가정
except ImportError:
    common_params = {}
...
class SentimentalAgent(BaseAgent):
    def __init__(self, ticker, agent_id="SentimentalAgent", news_dir=None, **kwargs):
        ...
        """
        # 1) kwargs 안에 들어온 news_dir를 먼저 꺼내서 BaseAgent로 안 올라가게 막기
        if "news_dir" in kwargs and news_dir is None:
            news_dir = kwargs.pop("news_dir")

        # 2) BaseAgent는 news_dir 모르는 애라서, 걸러진 kwargs만 넘겨야 함
        super().__init__(ticker=ticker, agent_id=agent_id, **kwargs)
        
        # 3) 이제 SentimentalAgent에서만 news_dir 처리
        if news_dir is None:
    ...
    def predict(self, X, n_samples: Optional[int] = None, current_price: Optional[float] = None):
        ...
        # 🔹 스케일러가 기대하는 feature 수에 맞춰 자동 조정
        n_expected = None
        try:
            x_scaler = getattr(self.scaler, "x_scaler", None)
            if x_scaler is not None and hasattr(x_scaler, "n_features_in_"):
                n_expected = int(x_scaler.n_features_in_)
        except Exception:
            n_expected = None

        if n_expected is not None and X_raw_np.shape[-1] != n_expected:
            print(
                f"[WARN] {self.agent_id} 입력 feature 수({X_raw_np.shape[-1]})와 "
                f"스케일러 기대값({n_expected}) 불일치 → 자동 조정합니다."
            )
            if X_raw_np.shape[-1] > n_expected:
                # 예: 현재 8, 스케일러는 5 → 앞 5개만 사용
                X_raw_np = X_raw_np[..., :n_expected]
            else:
                # 예: 현재 5, 스케일러는 8 → 부족분 0으로 패딩
                pad = n_expected - X_raw_np.shape[-1]
                pad_zero = np.zeros((*X_raw_np.shape[:-1], pad), dtype=X_raw_np.dtype)
                X_raw_np = np.concatenate([X_raw_np, pad_zero], axis=-1)
    ...
    def _predict_next_close(self) -> Tuple[float, float, float, List[str]]:
        ...
        # days=365는 run_dataset 내부에서 common_params를 참조하도록 변경됨
        if sd is None or getattr(sd, "X_seq", None) is None:
            sd = self.run_dataset(days=365) 

        target = self.predict(sd, n_samples=common_params.get("n_samples", 30))
